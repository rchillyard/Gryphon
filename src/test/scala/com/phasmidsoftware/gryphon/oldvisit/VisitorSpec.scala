/*
 * Copyright (c) 2023-2024. Phasmid Software
 */

package com.phasmidsoftware.gryphon.oldvisit

import org.scalatest.flatspec.AnyFlatSpec
import org.scalatest.matchers.should

import java.io.FileWriter
import scala.collection.immutable.Queue
import scala.util.Using

class VisitorSpec extends AnyFlatSpec with should.Matchers {
//
//  behavior of "PostVisitor"
//
//  it should "visitPost and ignore visitPre" in {
//    Using(PostVisitor[Int, Queue[Int]]()) {
//      target =>
//        val t1 = target.visitPost(1)
//        t1.journals.head shouldBe Seq(1)
//        val t2 = t1.visitPre(1)
//        t2 shouldBe t1
//        val t3 = t2.visitPost(2)
//        t3.journals.head shouldBe Seq(1, 2)
//    }
//  }
//
//  it should "visitPre twice to StringBuilder" in {
//    Using(PostVisitor[Int, StringBuilder]()) {
//      target =>
//        val t1 = target.visitPost(1)
//        t1.journals.head.toString shouldBe "1\n"
//        val t2 = t1.visitPre(1)
//    }
//  }
//
//  it should "implement visitPost twice and journal" in {
//    Using(SimpleVisitor.createPostQueue[Int]) {
//      target =>
//        val t1 = target.visitPost(1)
//        t1.journals.head shouldBe Seq(1)
//        val t2 = t1.visitPost(2)
//        t2.journals.head shouldBe Queue(1, 2)
//    }
//  }
//
//  it should "implement reversePost" in {
//    Using(SimpleVisitor.reversePost[Int]) {
//      target =>
//        val t1 = target.visitPost(1)
//        t1.journals.head shouldBe List(1)
//        val t2 = t1.visitPost(2)
//        t2.journals.head shouldBe List(2, 1)
//    }
//  }
//
//  behavior of "PreVisitor"
//
//  it should "visitPre twice" in {
//    Using(PreVisitor[Int, Queue[Int]]()) {
//      target =>
//        val t2 = target.visitPre(1)
//        t2 shouldBe PreVisitor(Queue(1))
//        t2.journals.head shouldBe Seq(1)
//        val t3 = t2.visitPre(2)
//        t3 shouldBe PreVisitor(Queue(1, 2))
//        t3.journals.head shouldBe Seq(1, 2)
//    }
//  }
//
//  it should "visitPre twice to StringBuilder" in {
//    Using(PreVisitor[Int, StringBuilder]()) {
//      target =>
//        val t1 = target.visitPre(1)
//        t1.journals.head.toString shouldBe "1\n"
//        val t2 = t1.visitPre(2)
//        t2.journals.head.toString shouldBe "1\n2\n"
//    }
//  }
//
//  it should "visitPre twice to FileWriter" in {
//    Using(PreVisitor[Int, FileWriter]()) {
//      target =>
//        val t1 = target.visitPre(1)
//        t1.journals.head.toString shouldBe "1\n"
//        val t2 = t1.visitPre(2)
//        t2.journals.head.toString shouldBe "1\n2\n"
//    }
//  }
//
//  it should "visitPre twice to named FileWriter" in {
//    val journal = new FileWriter("test.txt")
//    Using(PreVisitor[Int, FileWriter](journal)) {
//      target =>
//        val t1 = target.visitPre(1)
//        t1.journals.head.toString shouldBe "1\n"
//        val t2 = t1.visitPre(2)
//        t2.journals.head.toString shouldBe "1\n2\n"
//      // NOTE: no need to close the journal because it's closed by the visitor in a Using block.
//      // journal.close()
//    }
//    a[java.io.IOException] should be thrownBy journal.write("done")
//  }
//
//  it should "implement visitPre twice and journals.head" in {
//    Using(SimpleVisitor.createPre[Int]) {
//      target =>
//        val t1 = target.visitPre(1)
//        t1.journals.head shouldBe Seq(1)
//        val t2 = t1.visitPre(2)
//        t2.journals.head shouldBe Queue(1, 2)
//    }
//  }
//
//  it should "implement reversePre with two visits" in {
//    Using(SimpleVisitor.reversePre[Int]) {
//      target =>
//        val t1 = target.visitPre(1)
//        t1 shouldBe PreVisitor(List(1))
//        val t2 = t1.visitPre(2)
//        t2 shouldBe PreVisitor(List(2, 1))
//    }
//  }
//  behavior of "PostVisitorIterable"
//
//  it should "visitPost and ignore visitPre" in {
//    Using(PostVisitorIterable[Int, Queue[Int]]()) {
//      target =>
//        val t1: IterableVisitor[Int, Queue[Int]] = target.visitPost(1)
//        t1.iterator.toSeq shouldBe Seq(1)
//        val t2: IterableVisitor[Int, Queue[Int]] = t1.visitPre(1)
//        t2 shouldBe t1
//        val t3: IterableVisitor[Int, Queue[Int]] = t2.visitPost(2)
//        t3.iterator.toSeq shouldBe Seq(1, 2)
//    }
//  }
//
//  it should "implement visitPost twice and journal" in {
//    Using(SimpleVisitor.createPostQueue[Int]) {
//      target =>
//        val t1 = target.visitPost(1)
//        t1.iterator.toSeq shouldBe Seq(1)
//        val t2 = t1.visitPost(2)
//        t2.iterator.toSeq shouldBe Queue(1, 2)
//    }
//  }
//
//  it should "implement reversePost" in {
//    Using(SimpleVisitor.reversePostList[Int]) {
//      target =>
//        val t1 = target.visitPost(1)
//        t1.iterator.toSeq shouldBe List(1)
//        val t2 = t1.visitPost(2)
//        t2.iterator.toSeq shouldBe List(2, 1)
//    }
//  }
//
//  behavior of "PreVisitorIterable"
//
//  it should "visitPre twice" in {
//    Using(PreVisitorIterable[Int, Queue[Int]]()) {
//      target =>
//        val t2: IterableVisitor[Int, Queue[Int]] = target.visitPre(1)
//        t2.iterator.toSeq shouldBe Seq(1)
//        val t3: IterableVisitor[Int, Queue[Int]] = t2.visitPre(2)
//        t3.iterator.toSeq shouldBe Seq(1, 2)
//    }
//  }
//
//  it should "implement visitPre twice and journal" in {
//    Using(SimpleVisitor.createPreQueue[Int]) {
//      target =>
//        val t1 = target.visitPre(1)
//        t1.iterator.toSeq shouldBe Seq(1)
//        val t2 = t1.visitPre(2)
//        t2.iterator.toSeq shouldBe Queue(1, 2)
//    }
//  }
//
//  it should "implement reversePre with two visits" in {
//    Using(SimpleVisitor.reversePreList[Int]) {
//      target =>
//        val t1 = target.visitPre(1)
//        t1.iterator.toSeq shouldBe List(1)
//        val t2 = t1.visitPre(2)
//        t2.iterator.toSeq shouldBe List(2, 1)
//    }
//  }
//
//  behavior of "Visitor"
//
//  it should "function" in {
//    Using(PreVisitor[Int, Queue[Int]]()) {
//      target =>
//        val queue = Queue.empty[Int]
//        val a1: Option[Queue[Int]] = target.function(1)(queue)
//        a1 shouldBe Some(Queue(1))
//    }
//  }
//
//  it should "join 1" in {
//    Using(PreVisitor[Int, Queue[Int]]()) {
//      preVisitor =>
//        val postVisitor: PostVisitor[Int, Queue[Int]] = PostVisitor()
//        val target: SimpleVisitor[Int, Queue[Int]] = preVisitor join postVisitor
//        target.visitPre(1).journals.head shouldBe Queue(1)
//    }
//  }
//
//  it should "join 2" in {
//    Using(PreVisitor[Int, List[Int]]()) {
//      preVisitor =>
//        val postVisitor = SimpleVisitor.reversePost[Int]
//        val target: SimpleVisitor[Int, List[Int]] = preVisitor join postVisitor
//        val z1: SimpleVisitor[Int, List[Int]] = target.visitPre(1)
//        z1.visitPost(2).journals.head shouldBe List(2, 1)
//    }
//  }
//
//  it should "postFunc" in {
//    Using(PostVisitor[Int, Queue[Int]]()) {
//      target =>
//        val queue = Queue.empty[Int]
//        val a1: Queue[Int] = target.visitPre(1).journals.head
//        a1 shouldBe Queue(1)
//    }
//  }
}
