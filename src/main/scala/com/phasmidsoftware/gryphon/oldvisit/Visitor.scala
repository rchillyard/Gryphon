...
trait Visitor[A] extends AutoCloseable {
...
//  protected val journal: J
  ...
  def journalPre[J, X]: Journal[J, A, X]

  def journalPost[J, X]: Journal[J, A, X]
  
  def functionPre[X]: A => X

  def functionPost[X]: A => X
  
  def visitSimple(pre: Boolean)(a: A): Visitor[A]
  
  def visitPre[X](a: A): Visitor[A] = journalPre.addEntry(a)
...
//  protected def isPost: Boolean = !isPre
   ...
   */
//  protected def doVisit(a: A, doFunction: Boolean, pre: Boolean): Visitor[A, X, J] =
//    unit(
//      if (doFunction) {
//        journal.addEntry(journal, a)
...
// */
//trait SimpleVisitor[A, J] extends Visitor[A, A, J] {
//
//  /**
...
//   * @return an updated `Visitor[A, J]`.
//   */
//  override def visitPre(a: A): SimpleVisitor[A, J] =
//    doVisit(a, isPre, true).asInstanceOf[SimpleVisitor[A, J]]
//
//  /**
//   * Method to visit AFTER processing the (child) A values in DFS.
//   * For BFS, this method is a no-op.
...
//   * @return an updated Visitor[A, J].
//   */
//  override def visitPost(a: A): SimpleVisitor[A, J] =
//    doVisit(a, isPost, false).asInstanceOf[SimpleVisitor[A, J]]
//}
...
// */
//case class PreVisitor[A, J](journal: J, next: Option[SimpleVisitor[A, J]] = None)(implicit val ev: SimpleJournal[J, A]) extends AbstractVisitor[A, J](journal, next) {
//
//  /**
...
//   */
//  def unit(journal: J, maybeVisitor: Option[SimpleVisitor[A, J]]): SimpleVisitor[A, J] =
...
//   */
//  def unit(journal: J): SimpleVisitor[A, J] =
//    PreVisitor(journal)
//}
 ...
 *           Requires implicit evidence of type Journal[J, A].
 */
//case class PostVisitor[A, J](journal: J, next: Option[SimpleVisitor[A, J]] = None)(implicit val ev: SimpleJournal[J, A]) extends AbstractVisitor[A, J](journal, next) {
//
//  /**
...
// */
//case class PreVisitorIterable[A, J <: Iterable[A]](journal: J, next: Option[SimpleVisitor[A, J]] = None)(implicit val ev: IterableJournal[J, A]) extends AbstractIterableVisitor[A, J](journal, next) {
//  /**
...
// *           Requires implicit evidence of type IterableJournal[J, A].
// */
//case class PostVisitorIterable[A, J <: Iterable[A]](journal: J, next: Option[SimpleVisitor[A, J]] = None)(implicit val ev: IterableJournal[J, A]) extends AbstractIterableVisitor[A, J](journal, next) {
//  /**
...
// */
//case class PreKeyedVisitor[A, X, J](journal: J, next: Option[SimpleVisitor[A, J]] = None)(implicit val ev: KeyedJournal[J, A, X]) extends AbstractKeyedVisitor[A, X, J](journal, next) {
//  /**
...
// * @param ev      An implicit instance of the `KeyedJournal` type class, providing operations for keyed journals.
// */
//case class PostKeyedVisitor[A, X, J](journal: J, next: Option[SimpleVisitor[A, J]] = None)(implicit val ev: KeyedJournal[J, A, X]) extends AbstractKeyedVisitor[A, X, J](journal, next) {
//  /**
//   * A flag indicating whether the current visitor instance represents the "pre" state in the visiting process.
//   * This is used to manage the state and behavior of the visitor during traversal.
//   */
...
// */
//trait IterableVisitor[A, J <: Iterable[A]] extends SimpleVisitor[A, J] with IterableOnce[A] {
//  /**
...
// *           Requires implicit evidence of type Journal[J, A].
// */
//abstract class AbstractVisitor[A, J](journal: J, next: Option[SimpleVisitor[A, J]])(implicit val ava: SimpleJournal[J, A]) extends SimpleVisitor[A, J] {
//  self =>
//  /**
...
// */
//abstract class AbstractKeyedVisitor[A, X, J](journal: J, next: Option[SimpleVisitor[A, J]])(implicit val ava: KeyedJournal[J, A, X]) extends SimpleVisitor[A, J] {
//  self =>
//
//  /**
//   * Method to append a value of type `A` to a journal of type `J`, utilizing the implicit `Journal` type class instance.
...
// *           Requires implicit evidence of type IterableJournal[J, A].
// */
//abstract class AbstractIterableVisitor[A, J <: Iterable[A]](journal: J, next: Option[SimpleVisitor[A, J]] = None)(implicit val avai: IterableJournal[J, A]) extends AbstractVisitor[A, J](journal, next) with IterableVisitor[A, J] {
//  self =>
//
//  /**
