 ...
 *              Each edge connects a source vertex (`from`) and a target vertex (`to`),
...
case class EdgeList[V, E, Z](edges: Seq[Edge[E, V]]) extends SerializableGraph[V, E, Z] with Edges[V, E]:
   ...
   *
   * @return a sequence of `Triplet[V, E, Z]`, where each triplet describes the source vertex (`from`), 
   *         the target vertex (`to`), the edge attribute (`attribute`), and the edge type (`edgeType`) 
