...
object Kruskal:
   ...
   * @return a `TraversalResult[V, Edge[E, V]]` mapping each vertex to its incident MST edge
   *         (`None` for isolated vertices or the arbitrary first vertex processed).
   */
  def mst[V, E: {Numeric, Ordering}](graph: UndirectedGraph[V, E]): Seq[Edge[E, V]] =
    ...
    val sortedEdges: Seq[UndirectedEdge[E, V]] =
      graph.edges.collect { case e: UndirectedEdge[E, V] @unchecked => e }
              .toSeq
              .sortBy(_.attribute)

    // Fold over sorted edges, adding each to the MST if it connects two components.
    val (_, mstEdges) = sortedEdges.foldLeft((wuf0, Seq.empty[Edge[E, V]])) {
      case ((wuf, mst), edge) =>
        val u = edge.white
        val v = edge.black
