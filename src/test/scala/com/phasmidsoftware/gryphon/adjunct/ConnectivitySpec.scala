...
/**
 * Tests for ConnectivityASP, ConnectivityLazy, and ParentSize.
 *
 * Both implementations satisfy the same DisjointSet contract; they are tested
 * in parallel. ConnectivityLazy additionally provides balanced trees and
...
class ConnectivitySpec extends AnyFlatSpec with should.Matchers:
  ...
  // -------------------------------------------------------------------------
  // ConnectivityLazy — basic structure
  // -------------------------------------------------------------------------

  behavior of "ConnectivityLazy — structure"

  it should "create an empty ConnectivityLazy" in {
    val wuf = ConnectivityLazy.empty[Int]
    wuf.size shouldBe 0
  }

  it should "create singletons with create" in {
    val wuf = ConnectivityLazy.create(0, 1, 2, 3, 4)
    wuf.size shouldBe 5
  }

  it should "add a singleton with put" in {
    val wuf = ConnectivityLazy.empty[Int].put(0).put(1)
    wuf.size shouldBe 2
  }

  it should "report each vertex as its own root initially" in {
    val wuf = ConnectivityLazy.create(0, 1, 2)
  ...
  // -------------------------------------------------------------------------
  // ConnectivityLazy — connect / isConnected
  // -------------------------------------------------------------------------

  behavior of "ConnectivityLazy — connect"

  it should "connect two vertices" in {
    val wuf = ConnectivityLazy.create(0, 1, 2).connect(0, 1)
  ...
  it should "not change size when connecting already-connected vertices" in {
    val wuf = ConnectivityLazy.create(0, 1, 2).connect(0, 1).connect(0, 1)
    wuf.size shouldBe 2
  }

  it should "not change size when connecting a vertex to itself" in {
    val wuf = ConnectivityLazy.create(0, 1).connect(0, 0)
    wuf.size shouldBe 2
  }

  it should "transitively connect vertices" in {
    val wuf = ConnectivityLazy.create(0, 1, 2).connect(0, 1).connect(1, 2)
  ...
  it should "keep unconnected vertices separate" in {
    val wuf = ConnectivityLazy.create(0, 1, 2, 3).connect(0, 1).connect(2, 3)
    wuf.isConnected(0, 2) shouldBe false
    wuf.size shouldBe 2
  }

  // -------------------------------------------------------------------------
  // ConnectivityLazy — size and depth properties
  // -------------------------------------------------------------------------

  behavior of "ConnectivityLazy — size and depth"
  ...
  it should "track component size after merging two components" in {
    val wuf = ConnectivityLazy.create(0, 1, 2, 3)
  ...
  it should "keep tree depth bounded — depth never exceeds log2(n) + 1" in {
    ...
    val wuf = ConnectivityLazy.create(0, 1, 2, 3, 4, 5, 6, 7)
  ...
  behavior of "ConnectivityLazy — isRoot / parent / depth"
  ...
  it should "report a non-root correctly after connect" in {
    val wuf = ConnectivityLazy.create(0, 1).connect(0, 1)
  ...
  it should "remove a singleton and reduce component count" in {
    val wuf = ConnectivityLazy.create(0, 1, 2).remove(2)
    wuf.size shouldBe 2
  ...
  behavior of "ConnectivityLazy — getDisjointSet error"

  it should "throw GraphException for a missing key" in {
    val wuf = ConnectivityLazy.create(1, 2, 3)
    a[com.phasmidsoftware.gryphon.util.GraphException] should be thrownBy wuf.getDisjointSet(0)
  }

  // -------------------------------------------------------------------------
  // ConnectivityLazy — internal ParentSize structure after connect
  // -------------------------------------------------------------------------

  behavior of "ConnectivityLazy — internal structure"
  ...
  it should "attach smaller tree under larger and update sizes correctly" in {
    ...
    val c1 = ConnectivityLazy.create("A", "B", "C").connect("A", "B")
  ...
  behavior of "ConnectivityLazy — stress test"
  ...
  it should "correctly reduce components and maintain bounded depth" in {
    ...
    val target = ConnectivityLazy.create[Int](randomInts *)
    val connections = 4 * n
    val pairs = LazyList.continually(getPair).take(connections).toList
    val result = pairs.foldLeft(target)((u, t) => u.connect(t._1, t._2))
    result.size should be < n
    // ConnectivityLazy guarantees O(log n) depth — log2(1000) ≈ 10
  ...
  it should "have strictly lower maxDepth than ConnectivityASP on same data" in {
    ...
    val wuf = pairs.foldLeft(ConnectivityLazy.create[Int](randomInts *))((u, t) => u.connect(t._1, t._2))
    wuf.maxDepth should be <= uf.maxDepth
  }

  // -------------------------------------------------------------------------
  // ConnectivityOptimized — basic structure (same contract as ConnectivityLazy)
  // -------------------------------------------------------------------------
  ...
  // -------------------------------------------------------------------------
  // ConnectivityOptimized — connect / isConnected (same contract as ConnectivityLazy)
  // -------------------------------------------------------------------------
  ...
  it should "reduce maxDepth after a series of findAndCompress calls via connect" in {
    ...
    // findAndCompress internally) reduce depth compared to plain ConnectivityLazy.
    ...
    val c = chainPairs.foldLeft(ConnectivityLazy.create(nodes *))(
      (u, t) => u.connect(t._1, t._2))

    // ConnectivityOptimized should never be deeper than plain ConnectivityLazy
  ...
  it should "produce a compressed instance that is still functionally correct" in {
    ...
    // ConnectivityLazy invariants must still hold after compression
  ...
  it should "have maxDepth no greater than ConnectivityLazy on the same data" in {
    val rng = scala.util.Random(42)
    val n = 500
    val randomInts = LazyList.continually(rng.nextInt(10000)).take(n).toList
    ...
    val co = pairs.foldLeft(ConnectivityOptimized.create[Int](randomInts *))(
      (u, t) => u.connect(t._1, t._2))
    val wuf = pairs.foldLeft(ConnectivityLazy.create[Int](randomInts *))(
      (u, t) => u.connect(t._1, t._2))
