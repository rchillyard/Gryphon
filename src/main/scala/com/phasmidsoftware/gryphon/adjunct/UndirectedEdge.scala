...
case class UndirectedEdge[V, E](attribute: E, white: V, black: V) extends Edge[V, E]:
   ...
   */
  def other(v: V): V = if v == white then black else white

  /**
   * Symmetric equality: UndirectedEdge(a, u, v) == UndirectedEdge(a, v, u).
   * The default case class equals treats white and black as ordered, which
   * violates the mathematical definition of an undirected edge.
   */
  override def equals(obj: Any): Boolean = obj match
    case that: UndirectedEdge[?, ?] =>
      attribute == that.attribute && Set(white, black) == Set(that.white, that.black)
    case _ => false

  /**
   * hashCode consistent with symmetric equals — must be order-independent.
   */
  override def hashCode: Int =
    Set(white, black).hashCode ^ attribute.hashCode