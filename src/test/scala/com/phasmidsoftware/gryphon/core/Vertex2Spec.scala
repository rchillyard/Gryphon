...
class Vertex2Spec extends AnyFlatSpec with should.Matchers:

  behavior of "Vertex"

  val target: SimpleVertex[Int] = Vertex.create(1)
  ...
  it should "createWithBag" in {
    val v: Vertex[Int] = Vertex.createWithBag(2)
    v.attribute shouldBe 2
    v.adjacencies shouldBe Unordered_Bag(Bag.empty)
  }

  it should "createWithSet" in {
    val v: Vertex[Int] = Vertex.createWithSet(2)
    v.attribute shouldBe 2
    v.adjacencies shouldBe Unordered_Set(Set.empty)
  }

  it should "$plus 1 (Bag-backed vertex)" in {
    val vertex2 = Vertex.create(2)
    val result = vertex2 + AdjacencyVertex(2)
    result.attribute shouldBe 2
    val adjacencies = result.adjacencies.asInstanceOf[Unordered_Bag[Adjacency[Int]]]
  ...
  it should "$plus 2 (Set-backed vertex)" in {
    val vertex2 = Vertex.createWithSet(2)
    val result = vertex2 + AdjacencyVertex(1) + AdjacencyVertex(3) + AdjacencyVertex(4)
    result.attribute shouldBe 2
    val adjacencies = result.adjacencies.asInstanceOf[Unordered_Set[Adjacency[Int]]]
