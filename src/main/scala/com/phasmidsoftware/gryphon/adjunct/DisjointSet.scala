...
type ADS[V, W, This <: AbstractDisjointSet[V, W, This]] = AbstractDisjointSet[V, W, This]
 ...
 *
 * This abstract class does not perform path compression.
 * For path compression, you must extend the subclass `AbstractDisjointSetWithCompression`.
 *
...
(map: Map[V, W])(f: W => Option[V]) extends DisjointSet[V, This] with Connected[V]:
  ...
  def meanDepth: Double =
    map.keys.map(depth).sum.toDouble / map.size

  /** The maximum path length to root across all objects. */
  def maxDepth: Double =
    map.keys.map(depth).max.toDouble

  /**
   * Removes `key` from this disjoint set.
   */
  def remove(key: V): This =
    unit(map.removed(key))
   ...
   */
  def updated[W1 >: W](key: V, value: W1): Map[V, W1] =
  ...
  private[adjunct] def doMerge(v1: V, v2: V): This =
...
type ADSWC[V, W, This <: AbstractDisjointSetWithCompression[V, W, This]] = AbstractDisjointSetWithCompression[V, W, This]
 ...
 * @tparam This the concrete subtype (F-bounded polymorphism).
 */
abstract class AbstractDisjointSetWithCompression[V, W, This <: AbstractDisjointSetWithCompression[V, W, This]]
(map: Map[V, W])(f: W => Option[V]) extends ADS[V, W, This](map)(f):
   ...
   */
  def findAndCompress(key: V): (V, This)
   ...
   */
  override def connect(v1: V, v2: V): This =
    if v1 == v2
    then
