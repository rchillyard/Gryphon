...
/**
 * Tests for Connectivity, WeightedConnectivity, and ParentSize.
 *
 * Both implementations satisfy the same DisjointSet contract; they are tested
 * in parallel. WeightedConnectivity additionally provides balanced trees and
...
class ConnectivitySpec extends AnyFlatSpec with should.Matchers:
  ...
  it should "create an empty Connectivity" in {
    val uf = Connectivity.empty[Int]
    uf.size shouldBe 0
  }

  it should "create singletons with create" in {
    val uf = Connectivity.create(0, 1, 2, 3, 4)
    uf.size shouldBe 5
  }

  it should "add a singleton with put" in {
    val uf = Connectivity.empty[Int].put(0).put(1)
    uf.size shouldBe 2
  }

  it should "report each vertex as its own root initially" in {
    val uf = Connectivity.create(0, 1, 2)
  ...
  it should "connect two vertices" in {
    val uf = Connectivity.create(0, 1, 2).connect(0, 1)
  ...
  it should "not change size when connecting already-connected vertices" in {
    val uf = Connectivity.create(0, 1, 2).connect(0, 1).connect(0, 1)
    uf.size shouldBe 2
  }

  it should "not change size when connecting a vertex to itself" in {
    val uf = Connectivity.create(0, 1).connect(0, 0)
    uf.size shouldBe 2
  }

  it should "transitively connect vertices" in {
    val uf = Connectivity.create(0, 1, 2).connect(0, 1).connect(1, 2)
  ...
  it should "keep unconnected vertices separate" in {
    val uf = Connectivity.create(0, 1, 2, 3).connect(0, 1).connect(2, 3)
    uf.isConnected(0, 2) shouldBe false
    uf.size shouldBe 2
  }

  // -------------------------------------------------------------------------
  // WeightedConnectivity — basic structure
  // -------------------------------------------------------------------------

  behavior of "WeightedConnectivity — structure"

  it should "create an empty WeightedConnectivity" in {
    val wuf = WeightedConnectivity.empty[Int]
    wuf.size shouldBe 0
  }

  it should "create singletons with create" in {
    val wuf = WeightedConnectivity.create(0, 1, 2, 3, 4)
    wuf.size shouldBe 5
  }

  it should "add a singleton with put" in {
    val wuf = WeightedConnectivity.empty[Int].put(0).put(1)
    wuf.size shouldBe 2
  }

  it should "report each vertex as its own root initially" in {
    val wuf = WeightedConnectivity.create(0, 1, 2)
  ...
  // -------------------------------------------------------------------------
  // WeightedConnectivity — connect / isConnected
  // -------------------------------------------------------------------------

  behavior of "WeightedConnectivity — connect"

  it should "connect two vertices" in {
    val wuf = WeightedConnectivity.create(0, 1, 2).connect(0, 1)
  ...
  it should "not change size when connecting already-connected vertices" in {
    val wuf = WeightedConnectivity.create(0, 1, 2).connect(0, 1).connect(0, 1)
    wuf.size shouldBe 2
  }

  it should "not change size when connecting a vertex to itself" in {
    val wuf = WeightedConnectivity.create(0, 1).connect(0, 0)
    wuf.size shouldBe 2
  }

  it should "transitively connect vertices" in {
    val wuf = WeightedConnectivity.create(0, 1, 2).connect(0, 1).connect(1, 2)
  ...
  it should "keep unconnected vertices separate" in {
    val wuf = WeightedConnectivity.create(0, 1, 2, 3).connect(0, 1).connect(2, 3)
    wuf.isConnected(0, 2) shouldBe false
    wuf.size shouldBe 2
  }

  // -------------------------------------------------------------------------
  // WeightedConnectivity — size and depth properties
  // -------------------------------------------------------------------------

  behavior of "WeightedConnectivity — size and depth"
  ...
  it should "track component size after merging two components" in {
    val wuf = WeightedConnectivity.create(0, 1, 2, 3)
  ...
  it should "keep tree depth bounded — depth never exceeds log2(n) + 1" in {
    ...
    val wuf = WeightedConnectivity.create(0, 1, 2, 3, 4, 5, 6, 7)
  ...
  behavior of "WeightedConnectivity — isRoot / parent / depth"
  ...
  it should "report a non-root correctly after connect" in {
    val wuf = WeightedConnectivity.create(0, 1).connect(0, 1)
  ...
  it should "remove a singleton and reduce component count" in {
    val wuf = WeightedConnectivity.create(0, 1, 2).remove(2)
    wuf.size shouldBe 2
  ...
  it should "throw GraphException for a missing key" in {
    val uf = Connectivity.create(1, 2, 3)
    a[com.phasmidsoftware.gryphon.util.GraphException] should be thrownBy uf.getDisjointSet(0)
  }

  behavior of "WeightedConnectivity — getDisjointSet error"

  it should "throw GraphException for a missing key" in {
    val wuf = WeightedConnectivity.create(1, 2, 3)
    a[com.phasmidsoftware.gryphon.util.GraphException] should be thrownBy wuf.getDisjointSet(0)
  }

  // -------------------------------------------------------------------------
  // WeightedConnectivity — internal ParentSize structure after connect
  // -------------------------------------------------------------------------

  behavior of "WeightedConnectivity — internal structure"
  ...
  it should "attach smaller tree under larger and update sizes correctly" in {
    ...
    val c1 = WeightedConnectivity.create("A", "B", "C").connect("A", "B")
  ...
  behavior of "WeightedConnectivity — stress test"
  ...
  it should "correctly reduce components and maintain bounded depth" in {
    ...
    val randomInts = LazyList.continually(rng.nextInt(max)).take(n).toList
    def getPair: (Int, Int) = (randomInts(rng.nextInt(n)), randomInts(rng.nextInt(n)))
    val target = WeightedConnectivity.create[Int](randomInts*)
    val connections = 4 * n
    val pairs = LazyList.continually(getPair).take(connections).toList
    val result = pairs.foldLeft(target)((u, t) => u.connect(t._1, t._2))
    result.size should be < n
    // WeightedConnectivity guarantees O(log n) depth — log2(1000) ≈ 10
    result.maxDepth should be <= 20.0
  }

  it should "have strictly lower maxDepth than Connectivity on same data" in {
    val rng = scala.util.Random(42)
    val n = 500
    ...
    val uf  = pairs.foldLeft(Connectivity.create[Int](randomInts*))((u, t) => u.connect(t._1, t._2))
    val wuf = pairs.foldLeft(WeightedConnectivity.create[Int](randomInts*))((u, t) => u.connect(t._1, t._2))
